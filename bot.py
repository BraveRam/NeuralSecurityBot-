...
bot = telebot.TeleBot("6322877600:AAHDgKURdTKTMIX-phuYviNzvNLGoTwDBIw")

channel = ["@neuralp"]

WARN_MSG = "🤠Be Careful! This is your last chance⚠️"
WELCOME_MSG = "Hello, 👋\n\nWelcome to the group where you can unleash your creativity and passion for creating amazing telegram bots, ChatGPT bots and other projects using neural networks and programming skills. 🚀\n\nYou are part of a global family of developers who share the same vision and enthusiasm for building awesome things with code and AI. 🌎\n\nHere, you can showcase your projects, get feedback, collaborate with others, learn new tips and tricks, and have fun with your fellow neural programmers. 😊\n\nOur channel: @Neuralp is more than just a community, it's your home away from home, where you can find support, inspiration and friendship. 🏠\n\nWe are so happy to have you here and we can't wait to see what you will create next. 💯\n\nLet\'s make some magic happen! 🔥"

chbtn = InlineKeyboardMarkup()
hmmm = InlineKeyboardButton("📃Rules📃", url="https://t.me/neural_rules/2")
hm = InlineKeyboardButton("📢Channel📢", url="https://t.me/neuralp")
chbtn.add(hmmm)
chbtn.add(hm)
...
def gotcha(callback):    
    ...
    if not callback.from_user.id == user:
        ...
        if callback.data.startswith("add"):
        	...
        	msg = bot.send_message(chat_id=callback.message.chat.id, text=WELCOME_MSG, parse_mode="html", reply_markup=chbtn)
        	time.sleep(300)
...
def start(message):
	bot.send_message(message.chat.id, "✳️{} This Bot Is A Group Manager Of <b>@NeuralG</b>".format(message.chat.first_name), reply_markup = chatbtn(), parse_mode="html")
		...
		if message.reply_to_message:
			...
			if is_bot_admin == True:
				if is_group_admin == True:
					bot.send_message(message.chat.id, f"{message.from_user.first_name}: {WARN_MSG}", reply_to_message_id=message.reply_to_message.message_id)					
					bot.delete_message(message.chat.id, message.message_id)
				else:
...
def admin_commands(message):
	bot.reply_to(message, "Admin Commands:\n\n/mute - /unmute - /ban - /unban - /dban - /warn - /kick \n\nNB: All commands only works if you reply to specific user:)")
...
def handle_all(message):
	if bot.get_chat_member("@neuralp", message.from_user.id).status == "left":
		bot.delete_message(message.chat.id, message.message_id)
		bot.send_message(message.chat.id, f"⬛{message.from_user.first_name}, Kindly Join Our Channel before writing a message here:)", reply_markup=sub_btn())
	else:
		pass
	is_neural_group = is_neural_group_fun(message)
